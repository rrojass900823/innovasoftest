...
const MantenimientoCliente = () => {
    ...
    useEffect(() => {
        const initCarga = async () => {
            try {
                const resIntereses = await api.get('/api/Intereses/Listado');
                setIntereses(resIntereses.data);

                if (id) {
                    const resCliente = await api.get(`/api/Cliente/Obtener/${id}`);
                        ...
                        <Grid item xs={12} sm={8} sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            <Box sx={{ position: 'relative' }}>
                                ...
                                <IconButton
                                    color="primary"
                                    component="label"
                                    sx={{ position: 'absolute', bottom: -10, right: -10, backgroundColor: 'white', '&:hover': { backgroundColor: '#f5f5f5' } }}
                                >
